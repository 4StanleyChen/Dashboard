...
def doGet(request):
    ...
    if islogin:
        ...
        with connections['ResultDB'].cursor() as cursor:
            ...
            maxcomplete = ((originalMax[3] - 0.01) / 10 + 0.01)
            maxc_complete = ((originalMax[4] - 0.01) / 10 + 0.01)
            ...
            for i in range(len(gradeData)-1, -1, -1):
                allstudentgrade[gradeData[i] / maxgrade] += 1
            ...
            for i in range(len(avgRadar_percent)):
                ...
                Data[1] = '{:,.2f}'.format(originalMax[i])
                Data[2] = '{:,.2f}'.format(originalMax[i])
            ...
            for i in range(c):
                ...
                recommandcoursename = result[0].course_name
        ...
        if haveThisCourse is False:
            print('not have')
            to_render['IsLogin'] = 2
            return render(request, 'detailsAndChart.html', to_render)
